package com.cmpay.weixin.utils;
...
//            LOG.info("请求返回报文："+sb.toString());
    ...
    public static String getPrePayXml(WeiXinPrePay weiXinPrePay, String partnerKey){
        ...
        String sign=getPrePaySign(weiXinPrePay, partnerKey);//生成预支付请求签名
        ...
        sb.append("<trade_type>").append(weiXinPrePay.getTradeType()).append("</trade_type>");
        if(StringUtils.equals(WXConstants.TRADETYPE_NATIVE, weiXinPrePay.getTradeType())){
    ...
    public static String geWeiXintPrePaySign(String appid , String mch_id , String device_info , String trade_type , Map<String, Object> prePay , String partnerKey){
        ...
        if (!CmpayUtils.isEmpty(prePay.get("return_code"))) {
            preParams.put("return_code", prePay.get("return_code"));
        }
        if (!CmpayUtils.isEmpty(prePay.get("return_msg"))) {
            preParams.put("return_msg", prePay.get("return_msg"));
        }
        if (!CmpayUtils.isEmpty(prePay.get("appid"))) {
            preParams.put("appid", appid);
        }
        if (!CmpayUtils.isEmpty(prePay.get("mch_id"))) {
            preParams.put("mch_id", mch_id);
        }
        if (!CmpayUtils.isEmpty(prePay.get("device_info"))) {
            preParams.put("device_info", device_info);
        }
        if (!CmpayUtils.isEmpty(prePay.get("nonce_str"))) {
            preParams.put("nonce_str", prePay.get("nonce_str"));
        }
        if (!CmpayUtils.isEmpty(prePay.get("result_code"))) {
            preParams.put("result_code", prePay.get("result_code"));
        }
        if ("FAIL".equals(prePay.get("result_code"))) {
            if (!CmpayUtils.isEmpty(prePay.get("err_code"))) {
                preParams.put("err_code", prePay.get("err_code"));
            }
            if (!CmpayUtils.isEmpty(prePay.get("err_code_des"))) {
                preParams.put("err_code_des", prePay.get("err_code_des"));
            }
        }
        if (!CmpayUtils.isEmpty(prePay.get("trade_type"))) {
            preParams.put("trade_type", trade_type);
        }
        if (!CmpayUtils.isEmpty(prePay.get("prepay_id"))) {
            preParams.put("prepay_id", prePay.get("prepay_id"));
        }
        if (!CmpayUtils.isEmpty(prePay.get("code_url"))) {
    ...
    private static String getPrePaySign(WeiXinPrePay weiXinPrePay,String partnerKey){
        ...
        prePayMap.put("trade_type", weiXinPrePay.getTradeType()); // 交易类型
        prePayMap.put("product_id", weiXinPrePay.getProductId()); // 交易类型
